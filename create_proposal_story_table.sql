-- Proposal Story Feature - Database Schema
...
-- Create view that combines revision history with manual story entries
...
SELECT 
    ...
    CASE 
        WHEN change_category = 'FINANCIAL' THEN 'Financial Update'
        WHEN change_category = 'PROJECT_STATUS' THEN 'Status Change'
        WHEN change_category = 'TIMELINE' THEN 'Timeline Update'
        WHEN change_category = 'CLIENT_DETAILS' THEN 'Client Information Update'
        WHEN change_category = 'SOLUTION_SCOPE' THEN 'Solution Update'
        WHEN change_category = 'PERSONNEL' THEN 'Team Update'
        ELSE 'System Update'
    END as story_title,
    ...
    change_category as entry_category,
    business_impact as story_priority,
    changed_fields as story_metadata,
    NULL as parent_id,
    FALSE as is_manual,
    change_type as story_subtype
...
BEGIN
    ...
    SELECT 
        ...
        pt.story_metadata,
        pt.is_manual,
        -- Try to get user role from users table if it exists
        COALESCE(u.account_type, 'User') as author_role
    FROM proposal_timeline pt
    LEFT JOIN users u ON pt.author = u.username
