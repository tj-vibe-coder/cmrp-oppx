...
async function getBasicPresentationData(accountManager) {
    const metricsQuery = `
        ...
        WHERE account_mgr = $1
    `;
    const metricsResult = await db.query(metricsQuery, [accountManager]);
    ...
    const recentQuery = `
        ...
        LIMIT 10
    `;
    const recentResult = await db.query(recentQuery, [accountManager]);
    ...
    const highValueQuery = `
        ...
        LIMIT 5
    `;
    const highValueResult = await db.query(highValueQuery, [accountManager]);
