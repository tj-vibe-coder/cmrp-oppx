...
app.get('/api/snapshots/:type/:accountManager', async (req, res) => {
  console.log('[DEBUG] Account Manager Snapshot Request:', req.params.type, req.params.accountManager);
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'No snapshot found for account manager' });
    }
    res.json(result.rows[0]);
  } catch (error) {
    console.error('Failed to fetch account manager snapshot:', error);
